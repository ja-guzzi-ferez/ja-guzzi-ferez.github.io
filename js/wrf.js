...
document.addEventListener('DOMContentLoaded', function() {
  ...
  tabButtons.forEach(button => {
    button.addEventListener('click', function() {
      ...
      // Si se activa la pestaña del mapa, invalidar el tamaño del mapa
      if (targetTab === 'tab-mapa' && typeof map !== 'undefined') {
        setTimeout(() => {
          map.invalidateSize();
        }, 100);
      }
      ...
      if (targetTab === 'tab-Satelite') {
        if (!window.sateliteInitialized) {
          initSatelite();
          window.sateliteInitialized = true;
      ...
      if (targetTab === 'tab-ciclones') {
        if (!window.ciclonInitialized) {
          initCiclones();
          window.ciclonInitialized = true;
